...
在工状态审计系统，包含管理员网页端、员工网页端和员工端钉钉小程序演示版，当前版本已完成“项目管理 + 员工填报 + 管理员审批 + 统计展示 + 待审批撤回”的基础业务闭环。
...
- 员工网页端和钉钉小程序补齐撤回入口
- 管理员端统一展示“已撤回”状态
- 今日已填权重和剩余权重统计排除已驳回、已撤回记录
- 前端开发环境统一收敛到 `127.0.0.1:3000`
- 后端接口统一使用 `127.0.0.1:9090/api`
- 登录接口兼容 JSON 和表单提交
